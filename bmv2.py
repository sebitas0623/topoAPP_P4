...
class ONOSBmv2Switch(Switch):
    ...
    def __init__(self, name, json=None, debugger=False, loglevel="warn",
        ...
        #--------------- codigo de sebastian -----------------------------
        print os.getcwd()
        self.netcfgfile2 = os.getcwd() + '/bmv2-%s-netcfg.json' % self.name
        #-----------------------------------------------------------------
...
nodes {{
  ...
  node: {nodeId}
    ...
    def doOnosNetcfg(self, controllerIP):
        ...
        #------------------- Codigo de sebastian ------------------------
        print cfgData['devices'][self.onosDeviceId]['basic']['pipeconf']
        with open(self.netcfgfile2, 'w') as fp:
            json.dump(cfgData, fp, indent=4)
        #----------------------------------------------------------------
